...
这是一个可直接用 Visual Studio 或 VS Code 打开的静态 Web 项目，目标浏览器为 Google Chrome。项目包含欧美流行专辑测评、风格筛选、20 张随机专辑评分推荐、私人 Top 10、相似专辑推荐、登录注册、“联系我们”页面，以及系统时间、访问计数、广告图片切换和用户名本地存储登录功能。
...
- `login.html`：单独的用户名登录页面，保存用户名后跳转到主页
- `login.js`：用户名 localStorage 存储与跳转逻辑
- `site.js`：页眉登录状态、主页用户名显示、访问计数、系统时间、广告切换、横向循环字幕和滚动霓虹喷射效果
